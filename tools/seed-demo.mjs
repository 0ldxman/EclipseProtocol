...
await cover(
  ...
  [
    ...
    "Ни одна операция протокола не завершена. Часть закрыта, часть забыта, остальные продолжаются без нас.",
    ":::::",
    "",
    ":::::columns",
    "Полевые программы, отдельные выходы и всё, что за ними последовало. Записи раздела ведутся по донесениям групп и сверяются с журналом связи.",
    "",
    "::::right",
    ":::fields",
    "записей :: 9",
...
await mk(
  ...
  [
    ...
    "",
    "Оперативник полевой группы, участник [[Протокол Аполлон|apollo]]. Последний человек, видевший станцию [[Мыс Тишина|cape]] до её закрытия.",
    "",
    "## Биография",
    "",
    "Родился в промышленном поясе. Служил в разведке, вербован после операции «Северный Ветер» — по представлению [[Отдел «Восход»|voskhod]].",
    "",
    ":::note{style=info}",
    ...
    "### Ранние годы",
    "",
    "Ничего примечательного, кроме результатов стрелковой подготовки: три года подряд первое место по округу.",
    "",
    "## Операции",
    "",
    "| год | операция | роль |",
    ...
    "",
    "Последние известные позывные группы:",
    "«Кремень» — старший",
    "«Сова» — связь",
    "«Пила» — :spoiler[имя изъято]",
    "",
    "Личное дело: __том III, лист 41__.",
  ].join("\n"),
  {
    slug: "kremen",
    tags: ["оперативники", "разлом", "пропавшие"],
    eventAt: "2031-04-12",
    eventEpoch: "Разлом",
    eventSummary: "Последний сеанс связи с группой. Дальше — только поисковые сводки.",
    eventImage: portrait,
  },
);
...
await mk(
  ...
  [
    ...
    "",
    "Связист группы. Единственная, кто вёл журнал связи вручную — благодаря чему хронология [[Протокол Аполлон|apollo]] вообще восстановима.",
    "",
    "## Журнал",
    "",
  ...
  {
    slug: "sova",
    tags: ["оперативники", "связь"],
    eventAt: "2029-08-02",
    eventEpoch: "Восхождение",
    eventSummary: "Связистка уходит на «Аполлон» и заводит журнал, по которому потом восстановят всю хронологию.",
  },
);
...
await mk(
  ...
  ].join("\n"),
  {
    ...
    eventAt: "2033-01-19",
    eventEpoch: "Пепел",
    eventSummary: "Сводный том закрыт и передан в архив: работа над протоколом прекращена.",
  },
);
...
await mk(
  ...
  {
    slug: "apollo",
    tags: ["операции", "восхождение"],
    eventAt: "2029-08-02",
    eventEpoch: "Восхождение",
    eventSummary: "Совместный выход трёх сторон. Единственная высадка программы, прошедшая без потерь.",
    eventImage: field,
  },
);
...
await mk(
  ...
  {
    slug: "eden-op",
    tags: ["операции", "расселение", "разлом"],
    eventAt: "2030-03-11",
    eventEpoch: "Разлом",
    eventSummary: "Вторая площадка расселения принимает первые семьи.",
    eventImage: camp,
  },
);
...
await mk(
  ...
  {
    slug: "sad",
    tags: ["расселение", "разлом"],
    eventAt: "2031-09-12",
    eventEpoch: "Разлом",
    eventSummary: "Радиоперехват: «Сад закрыт». Программу сворачивают за одни сутки.",
  },
);
...
await mk(
  ...
  ].join("\n"),
  {
    ...
    eventAt: "2031-06-03",
    eventEpoch: "Разлом",
    // Запись под грифом: подпись и снимок у события заполнены, но в ленту не
    // попадут — на них тот же допуск, что и на тело.
...
await mk(
  ...
  {
    slug: "cape",
    tags: ["места", "восхождение"],
    eventAt: "2027-05-30",
    eventEpoch: "Восхождение",
    eventSummary: "Станция на мысе выходит на постоянное дежурство.",
    eventImage: station,
  },
);
...
await mk(
  ...
  ].join("\n"),
  {
    ...
    eventAt: "2026-11-04",
    eventEpoch: "",
    eventSummary: "Отдел учреждён приказом по архивной службе — за год до первых событий протокола.",
  },
);
...
await mk(
  ...
  ].join("\n"),
  {
    ...
    eventAt: "2032-02-08",
    eventEpoch: "Пепел",
    eventSummary: "Коллегия прекращает поиски группы «Кремня».",
  },
);
...
await mk(
  ...
  ].join("\n"),
  {
    ...
    eventAt: "2028-06-15",
    eventEpoch: "Восхождение",
    eventSummary: "Испытания изделия 7: из двенадцати передатчиков работают два.",
    eventImage: device,
  },
);
