 ...
 * Летопись.
 *
 * Лента собирается из самих записей: запись объявляет себя событием
 * директивой `::event`. Отдельного хранилища у хронологии нет, поэтому она
  ...
  if (data.events.length === 0) {
    page.append(
      el("div", { class: "empty" }, [
        el("b", {}, ["летопись пуста"]),
        el("span", {}, [
          "Событие появится здесь, как только запись объявит себя им:",
          el("br"),
          "::event{at=2022-07-11 epoch=«Разлом»}",
        ]),
      ]),
    );
  } else {
