...
/** Socket URL for a room, derived from the page URL so path proxies keep working. */
export function collabUrl(room: string): string {
  const url = new URL(window.location.href);
  url.protocol = url.protocol === "https:" ? "wss:" : "ws:";
  url.hash = "";
  url.search = "";
  url.pathname = `${serverBase(url.pathname)}/collab/${room}`;
  return url.toString();
}
